...
一般连起来称为渲染管线。渲染不是一下子完成的，根据任务目标（坐标变换，光照计算什么的）和硬件执行特点，标准制定者制定了渲染过程的多个步骤，它们就像制造工厂的流水线一样，一个步骤对数据处理后的结果，交给下一个步骤处理，每个步骤连起来，就成了一个流水线，这个流水线就是渲染管线。
...
好，回到这一节的代码，我们现在要告诉显卡怎么“画”，就需要写自己的shader。
